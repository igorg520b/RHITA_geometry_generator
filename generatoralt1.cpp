...
void GeneratorAlt1::Generate()
{
    ...
    int pt1 = gmsh::model::occ::addPoint(0, blockHeight ,0);
    int pt2 = gmsh::model::occ::addPoint(0.5, blockHeight ,0);
    int pt3 = gmsh::model::occ::addPoint(blockLength, blockHeight ,0);
    int pt4 = gmsh::model::occ::addPoint(blockLength, 0 ,0);
    int pt5 = gmsh::model::occ::addPoint(1.5, 0 ,0);

    int line1 = gmsh::model::occ::addLine(pt0,pt1);
    int line2 = gmsh::model::occ::addLine(pt1,pt2);
    int line3 = gmsh::model::occ::addLine(pt2,pt5);
    int line4 = gmsh::model::occ::addLine(pt5,pt0);

    int line5 = gmsh::model::occ::addLine(pt2,pt3);
    int line6 = gmsh::model::occ::addLine(pt3,pt4);
    int line7 = gmsh::model::occ::addLine(pt4,pt5);
    int line8 = -line3;

    int loopTag1 = gmsh::model::occ::addCurveLoop({line1,line2,line3,line4});
    gmsh::model::occ::addPlaneSurface({loopTag1});
    int loopTag2 = gmsh::model::occ::addCurveLoop({line5,line6,line7,line8});
    gmsh::model::occ::addPlaneSurface({loopTag2});

    gmsh::model::occ::synchronize();
    int groupTag1 = gmsh::model::addPhysicalGroup(2, {loopTag1});
    int groupTag2 = gmsh::model::addPhysicalGroup(2, {loopTag2});
    int groupTag3 = gmsh::model::addPhysicalGroup(1, {line4, line7});
    ...
    nodeTags.clear();
    nodeCoords.clear();
    gmsh::model::mesh::getNodesForPhysicalGroup(2, groupTag1, nodeTags, nodeCoords);
    for(unsigned j=0;j<nodeTags.size();j++) mesh2d.nodes[mtags[nodeTags[j]]]->group|=1;
    spdlog::info("groupTag1 nodes {}",nodeTags.size());

    nodeTags.clear();
    nodeCoords.clear();
    gmsh::model::mesh::getNodesForPhysicalGroup(2, groupTag2, nodeTags, nodeCoords);
    for(unsigned j=0;j<nodeTags.size();j++) mesh2d.nodes[mtags[nodeTags[j]]]->group|=2;
    spdlog::info("groupTag2 nodes {}",nodeTags.size());

    nodeTags.clear();
    nodeCoords.clear();
    gmsh::model::mesh::getNodesForPhysicalGroup(1, groupTag3, nodeTags, nodeCoords);
    for(unsigned j=0;j<nodeTags.size();j++) mesh2d.nodes[mtags[nodeTags[j]]]->group|=4;
    spdlog::info("groupTag3 nodes {}",nodeTags.size());
    ...
    for(std::size_t i=0;i<trisTags.size();i++)
    {
        ...
        if((elem->nds[0]->group & 1) && (elem->nds[1]->group & 1) && (elem->nds[2]->group & 1)) elem->grainId = 1;
        else elem->grainId = 2;
    }
    ...
    spdlog::info("GeneratorAlt1::Generate() done");
}

void GeneratorAlt1::CreateCDP(std::ofstream &s)
{
...
void GeneratorAlt1::CreatePy()
{
    // SAVE as .py
    std::ofstream s;
    std::string fileName = outputFileName+".py";
